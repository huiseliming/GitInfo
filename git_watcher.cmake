...
function(GetGitState _working_dir)
    ...
    RunGitCommand(rev-list HEAD --count --first-parent)
    if(NOT exit_code EQUAL 0)
        set(ENV{GIT_BUILD_VERSION} "\"?\"")
    else()
        set(ENV{GIT_BUILD_VERSION} "\"${output}\"")
        set(ENV{GIT_PATCH_VERSION} "\"${output}\"")
    endif()
    
    RunGitCommand(describe HEAD --tags --abbrev=0 --match="v*" --always)
    if(${output} MATCHES "^v.*$")
        set(ENV{GIT_TAG_VERSION} "${output}")
        RunGitCommand(rev-list ${ENV{GIT_TAG_VERSION}}..HEAD --count)
        if(NOT exit_code EQUAL 0)
            set(ENV{GIT_PATCH_VERSION} "\"?\"")
        else()
            set(ENV{GIT_PATCH_VERSION} "\"${output}\"")
        endif()
    else()
        set(ENV{GIT_TAG_VERSION} "\"v0.0\"")
    endif()
